/**
 * خدمات — لوحة تحكم المستقل الكاملة.
 */
...
import type { UserRole } from '@/lib/services/user-meta';
...
export async function FreelancerDashboard({ kycPending = false }: { kycPending?: boolean } = {}) {
  ...
  const dashboardRole = currentUser.role as UserRole;
  const [stats, latestProjects, proposalCounts, contracts, wallet, profile] = await Promise.all([
    getDashboardStats(currentUser.id, dashboardRole),
    getLatestProjects(currentUser.id, dashboardRole, 5),
    getDashboardProposalCounts(currentUser.id, dashboardRole),
    getMyContracts(),
