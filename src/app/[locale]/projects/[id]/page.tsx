...
export default async function ProjectDetailsPage({ params }: ProjectDetailsPageProps) {
  ...
  const [proposals, alreadyProposed, wishlistedProjects] = await Promise.all([
            ...
            <article className="rounded-[14px] border border-gray-200 bg-white p-6 shadow-sm sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <FavoriteButton type="project" id={project.id} isLoggedIn={Boolean(currentUser)} initialFavorited={wishlistedProjects.has(project.id)} size="md" />
                  <span
                    className={`rounded-full px-3 py-1 text-[11px] font-bold ${PROJECT_STATUS_BADGE_CLASSES[project.status as keyof typeof PROJECT_STATUS_BADGE_CLASSES]}`}
                  >
